...
Node* AVL::LLRotation(Node *p) {
  ...
  /*     p                                                                         pl
       /   \                                                                     /   \
      pl.   any node(represent with 0).        after rotation.                  0.    p
      /\                                                                             / \
     0. plr                                                                         plr 0 */
    pl->rchild = p;
    p->lchild = plr;
 
    // modify the height of p and pl
    p->height = NodeHeight(p);
    pl->height = NodeHeight(pl);
 
    // Update root if root and p are same then root equal to new root ie pl
    if (root == p){
        root = pl;
    }
    return pl;
...
Node* AVL::rInsert(Node *p, int key) {
  ...
  // decide which side we have to insert the data
    if (key < p->data){
